...
1)  Run `install.packages('rsconnect')` and `library(rsconnect)` in your Console.

2)  Go to <https://www.shinyapps.io/> and create an account (click Sign Up). Follow the instructions. You can use the same username as your GitHub account if you'd like to be consistent.

3)  Click Show Secret, and copy the Account Authorization into your R Console, with <SECRET> filled in

    ```         
    rsconnect::setAccountInfo(name='grcetmpk',
                  token='E60FD358D5080B343605C63AE78C7858',
                  secret='<SECRET>')
    ```

4)  Deploy the app by running in your Console:

    ```         
    rsconnect::deployApp('[PATH TO YOUR APP FOLDER]')
    ```

-   for example, to deploy my BC Liquor demo app, I'd insert the path `/Users/gracetompkins/Desktop/BCLiquor` (which is the folder where app.R exists).
