---
title: "Lecture 3: Reproducibility with RMarkdown"
...
-  One way to ensure reproducible research is sharing clear details on the analysis and providing the code used to produce the results.

-  Reproducibility is often confused with *replication*. However, replication is a separate concept where the results of a study should be validated on an independent study using new data. 

-  A "good" study is both replicable and reproducible.


## Reproducibility


-  There are also ethical benefits to reproducible research. Performing open research can reduce the chance of data fabrication or other ethical issues such as *p-hacking*, where a researcher tests a number of hypotheses until they find one that is statistically significant. 

-  Open, transparent research poses a sense of accountability on the researcher(s). 

-  Of course, you can't always share data (for example, health data containing personal identifying information), but you should share as much as is possible. Always check with your ethics board or principal investigator before sharing data online.
...
-  Reproducible research can also force you to have better, more automated workflows. 

-  The first analysis I (Grace) ever did in R, I was manually changing things in excel documents, and then saving them in a certain place, and then using R to fit a model, and then exporting the data, and then changing the worksheet format, and so on. 

-  I had a sticky note of instructions on how to produce the results on my desk. This was neither reproducible nor productive for my time. While automating some steps and using tools like GitHub for version control can be more work upfront, it can save you *a lot* of headaches down the road. It is something I personally wish I learned to use earlier in my career.
...
Take 15 minutes to start this tutorial, and then we will return to the lecture. 
...
-  The key drivers under the hood are `knitr`and `Pandoc`! 

-  When you press “Knit”, R Markdown passes the .`Rmd`file to `knitr`, which executes all of the code and creates a new `.md` file including the code and output. 

-  Then, that `.md` file is processed into the final output format (e.g. `.html`) by pandoc.
...
We will now demonstrate a few basics of RMarkdown using the `mtcars` dataset, including

-  Headers

-  Comments

-  Code chunks (with options like echo and warning)

-  Lists

-  In-line r code using \`r `
:::
