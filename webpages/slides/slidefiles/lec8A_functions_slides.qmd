...
-   standard test cases (i.e., rolling two, 3, or 100 die), and

-   e*dge cases* (conditions that fall outside the typical or expected parameters, i.e. rolling 2.5 die)
...
Within a function call, we can force errors to appear using the `stop()` function and conditional statements. F
...
roll_d10 <- function(num_dice) { 
    ...
    if(num_dice ...FILLTHISIN..){ #if num_dice is not an integer
...
*(Aside: we can output lists using `list()` and name the list for even "prettier" output. But this is beyond the scope of the course at this point)*
