...
[**Due September 19th, 2025 at 11:59pm PT**]{.underline}
...
::: callout-important
**Read through the project in its entirety** to ensure you've made a plan for all of the components to be completed! Many tasks are not "standalone". For example, we require at least one pull request per person that is relevant to the project. You'll need to decide in advance who does what!
:::
...
1.  Create a Group Chat on our Slack Team with all of your members in it (see Canvas for the link to join if you haven't already done so)

2.  Have one group member make a repository on GitHub, by following the steps found on the [Collaborative Project assignment on Canvas.](https://canvas.ubc.ca/courses/169555/assignments/2225471)

    -   Your team name should be your group number.

    -   Once completed, your repository should be in the GitHub classroom.

    -   You may want to save the link to this repository so you can access it later.
...
-   4 or 5 markdown features: 13 points

-   5 markdown features, with at least two being "GitHub-Flavoured": 15 points

## Exercise 3: Merge one Pull Request per Teammate

**30 points**

**Every teammate should submit at least one pull request for this milestone, and have a different teammate merge the pull request after reviewing the contents of the pull request**. **Each pull request will require a number of components (listed below). TAs will grade one pull request per person, and it's up to you to ensure you've indicated which one is to be graded by adding "(GRADED)" to your pull request name.**

So, like the previous exercise on Markdown Formatting, this exercise is not stand-alone. You’ll need to decide who will work on what (more on this when you make your teamwork document).
...
1.  Make a new branch, *before you start working on your tasks!* (So that you capture your work within the branch.)
    ...
    -   You can do this either on GitHub, or on your own computer using RStudio.
...
3.  Start a pull request on GitHub, by going to “Pull Requests” -\> “New Pull Request”, and selecting the branch you intend to merge to the main branch. In your pull request, include a comment and title indicating (at a high level) what this change you made is about.

-   In the comments, include some indication as to which teammate will be responsible for checking and merging the new work contained in the pull request.
...
-   The teammate responsible for reviewing and merging the pull request does so, being sure to add to the discussion happening in the pull request by adding a comment.
...
**Note 2:** Please indicate to the TAs which Pull Requests you would like to be graded by adding “(GRADED)” to the PR name. Every team member should have exactly one pull request labelled "(GRADED)".
...
1.  Put the [`troubleshooting-1.Rmd` document](https://github.com/UBC-STAT/STAT545.github.io/blob/main/content/collaborative-project/milestone1/troubleshooting-1.Rmd) in your repository. (You can either download this file, or just copy and paste its contents into a new file that you call `troubleshooting-1.Rmd`.)
...
-   Put a link to that release as a submission on the Collaborative Project Milestone 1
